...
MAX_RETRIES_BY_RECOVERABLE: dict[str, int] = {
    ...
    RecoverableSubtype.PLANNER.value: 3,
...
BACKOFF_PROFILE_BY_SUBTYPE: dict[str, BackoffProfile] = {
    ...
    RecoverableSubtype.PLANNER.value: BackoffProfile(base_seconds=1.0, cap_seconds=30.0),
...
def compute_backoff_seconds(
    ...
    *,
    ...
    """
    指数退避：base * 2**attempt_index + 抖动；再由 ``settings`` 与 subtype cap 限制。
    attempt_index 从 0 开始（首次恢复等待）。
    """
    jitter_ratio = jitter_ratio if jitter_ratio is not None else settings.backoff_jitter_ratio
    ...
    env_cap = float(settings.backoff_max_seconds)
    raw = min(raw, env_cap)
