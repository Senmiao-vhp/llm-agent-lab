...
class TestPolicy(unittest.TestCase):
    ...
    def test_backoff_monotonic_no_jitter(self) -> None:
        ...
        s0 = compute_backoff_seconds(
            RecoverableSubtype.NETWORK.value, 0, jitter_ratio=0.0, rng=rng
        )
        rng = random.Random(42)
        s1 = compute_backoff_seconds(
            RecoverableSubtype.NETWORK.value, 1, jitter_ratio=0.0, rng=rng
        )
    ...
    def test_backoff_respects_cap(self) -> None:
        ...
        s = compute_backoff_seconds(
            RecoverableSubtype.RATE_LIMIT.value, 20, jitter_ratio=0.0, rng=rng
        )
