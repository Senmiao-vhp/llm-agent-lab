...
- **生成时间**：2026-05-15T21:18:32.659556
- **金标文件**：`D:\Users\资料夹\智能体\llm-agent-lab\app3\data\eval_runs\eval_pipeline_retry_tf_2.jsonl`
- **主意图过滤**：`（未过滤）`
- **样本条数**：2
...
- **任务成功率 `success_rate`**：0.5
- **意图准确率 `intent_acc`**：1.0
- **槽位三项合取准确率 `slot_acc`**：1.0
- **行政区单项 `regions_acc`**（有 `regions_match` 的样本）：1.0
- **时间单项 `time_range_acc`**：1.0
- **对象单项 `target_object_acc`**：1.0
- **响应时间 p50（ms）**：27544.456
- **响应时间 p90（ms）**：35264.246

## 4. 分意图统计（按 `gold.primary_intent`）

| 金标意图 | 条数 | intent_acc | slot_acc（三项全对占比） |
|----------|-----:|-----------:|-------------------------:|
| `transfer_analysis` | 2 | 1.0 | 1.0 |

## 5. 逐条结果（总表）

| sample_id | duration_ms | task_success | intent_match | regions | time | target | phase |
|-----------|------------:|--------------|--------------|---------|------|--------|-------|
| eval210_tf_003 | 17894.719 | False | True | True | True | True | explain |
| eval210_tf_004 | 37194.193 | True | True | True | True | True | explain |
...
- **`eval210_tf_003`**：phase=`explain` error_class=`none` fatal=`None` checks={"intent_match": true, "regions_match": true, "time_range_match": true, "target_object_match": true}
  - **`gis_pipeline`**（GIS 摘要）：`{"success": false, "message": "Task failed: q1_geom (resolve_geometry): Failed to resolve geometry for region='安徽省合肥市'.\n- GEE/GAUL: No GAUL match found.\n- local geoBoundaries: no CHN admin match in geoBoundaries for '安徽省合肥市'. Last: 'GeoBoundariesDownloader' object has no attribute '_raw_admin_boundary_dir'\n- Nominatim: Nominatim: no boundary for '安徽省合肥市'. Last error: RuntimeError(\"no polygon in geojson for query='安徽省合肥市'\")"}`
