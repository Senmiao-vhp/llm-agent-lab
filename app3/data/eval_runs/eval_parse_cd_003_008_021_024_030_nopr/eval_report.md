...
- **生成时间**：2026-05-16T12:15:08.042927
...
- **槽位三项合取准确率 `slot_acc`**：0.6
- **行政区单项 `regions_acc`**（有 `regions_match` 的样本）：1.0
- **时间单项 `time_range_acc`**：0.6
- **对象单项 `target_object_acc`**：1.0
- **响应时间 p50（ms）**：3279.019
- **响应时间 p90（ms）**：5214.386

## 4. 分意图统计（按 `gold.primary_intent`）

| 金标意图 | 条数 | intent_acc | slot_acc（三项全对占比） |
|----------|-----:|-----------:|-------------------------:|
| `change_detection` | 5 | 0.6 | 0.6 |

## 5. 逐条结果（总表）

| sample_id | duration_ms | task_success | intent_match | regions | time | target | phase |
|-----------|------------:|--------------|--------------|---------|------|--------|-------|
| eval210_cd_003 | 6189.031 | True | True | True | False | True | parse |
| eval210_cd_008 | 3752.418 | True | False | True | False | True | parse |
| eval210_cd_021 | 3279.019 | True | True | True | True | True | parse |
| eval210_cd_024 | 3003.222 | True | True | True | True | True | parse |
| eval210_cd_030 | 3222.429 | True | False | True | True | True | parse |
